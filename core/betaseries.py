...
class EpisodeObject():
    ...
    def __init__(self, tvdbid, tvdbepid, showtitle, epname, showalreadyadded, episodealreadydownloaded):
        self.tvdbid = tvdbid
...
class BetaSeriesDownloaded:
    ...
    def _get_info(self, showfile):
        xbmc.log(settings.LOG_ADDON_NAME + "_get_info", xbmc.LOGDEBUG)
        ...
        if len(infosshow) > 0:
            ...
            try:
                ...
                # xbmc.log(settings.LOG_ADDON_NAME + "Infos got from betaseries " + tvdbid)
            except:
            ...
            #pass the user !!!!!!!
            urldata = [['v', settings.API_VER], ['key', settings.API_KEY], ['thetvdb_id', str(tvdbid)], ['season', infosshowseason], ['episode', infosshowepisode]]
            try:
                ...
                xbmc.log(settings.LOG_ADDON_NAME + "could not fetch episode's thetvdb_id | " + url + urldata,
                         xbmc.LOGNOTICE)
                return False

            alreadyadded = False
            epinfo = EpisodeObject(int(tvdbid), int(tvdbepid), showtitle, epname, alreadyadded, downloaded)
    ...
    def _service_betaserie(self, file):
        ...
        if not self.betaserieobject.getAuthFail():
            ...
            if self.betaserieobject.getToken():
                ...
                if episode.getTvdbId() and episode.getTvdbEpId():
    ...
    def _service_mark_as_downloaded(self, episode):
        ...
        xbmc.log(settings.LOG_ADDON_NAME + "episode: " + str(episode).strip('[]'))
        ...
        if not episode.getEpisodeAlreadyDownloaded():
            ...
            if settings.BETA_FOLLOW and not episode.getEpisodeAlreadyDownloaded():
                ...
                if data['errors']:
                    ...
                    if settings.BETA_NOTIFY:
                        xbmc.executebuiltin((u'Notification(%s,%s,%s,%s)' % (
                            settings.ADDON_NAME, settings.ADDON_TRAD(30013) + episode.getShowTitle().decode('utf-8'), 750, settings.ADDON_ICON)).encode(
                            'utf-8',
                            'ignore'))
                ...
                if settings.BETA_NOTIFY:
                    xbmc.executebuiltin(
                        (u'Notification(%s,%s,%s,%s)' % (
                            ...
                            'utf-8', 'ignore'))
                xbmc.log(settings.LOG_ADDON_NAME + "%s episode %s marked as downloaded" % (episode.getShowTitle(), episode.getEpName()))
