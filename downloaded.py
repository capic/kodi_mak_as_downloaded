...
def set_user_agent():
    xbmc.log(__addonname__ + "=> set_user_agent")
    ...
    try:
        ...
        xbmc.log(__addonname__ + "=> could not get app version")
        version = "XBMC"
    return "Mozilla/5.0 (compatible; " + __platform__ + "; " + version + "; " + __addonid__ + "/" + __addonversion__ + ")"


def get_urldata(url, urldata, method):
    xbmc.log(__addonname__ + "=> IN ***** get_urldata *****", xbmc.LOGDEBUG)

    xbmc.log(__addonname__ + "=> url: %s, urldata: %s, method: %s" % (url, urldata, method), xbmc.LOGDEBUG)
    ...
    if connection.code:
        ...
        xbmc.log(__addonname__ + "=> OUT ***** get_urldata *****", xbmc.LOGDEBUG)
        return response
    else:
        log('response empty')
...
class Main:
    ...
    def _get_settings(self):
        xbmc.log(__addonname__ + "=> Main._get_settings")
        xbmc.log(__addonname__ + "=> reading settings")
        service = []
        ...
        if betaactive and betauser and betapass and path:
            ...
            if service[11]:
                xbmc.executebuiltin(
                    (u'Notification(%s,%s,%s,%s)' % (__addonname__, __language__(30003), 750, __icon__)).encode('utf-8',
                                                                                                                'ignore'))

    # episode: [int(tvdbid), int(tvdbepid), int(playcount), showtitle, epname, downloaded]
    # [service, api-url, api-key, user, pass, token, auth-fail, failurecount, timercounter, timerexpiretime, follow, notify]
    def _service_betaserie(self, episode, service):
        xbmc.log(__addonname__ + "=> Main._service_betaserie")
    ...
    def _service_authenticate(self, service, timestamp):
        xbmc.log(__addonname__ + "=> Main._service_authenticate")
        ...
        try:
            ...
            xbmc.log(__addonname__ + "=> successfully authenticated")
        except:
            service = self._service_fail(service, True)
            xbmc.executebuiltin(
                (u'Notification(%s,%s,%s,%s)' % (__addonname__, __language__(32003), 750, __icon__)).encode('utf-8',
                                                                                                            'ignore'))
            xbmc.log(__addonname__ + "=> failed to connect for authentication", xbmc.LOGNOTICE)
            return service
        # parse results
        if 'token' in data:
            # get token
            ...
            service[9] = 0
        if data['errors']:
            xbmc.log(__addonname__ + "=> %s error %s : %s" % (
                service[0], data['errors'][0]['code'], data['errors'][0]['text']), xbmc.LOGNOTICE)
            if data['errors'][0]['code'] < 2000:
                # API error
                xbmc.executebuiltin(
                    (u'Notification(%s,%s,%s,%s)' % (__addonname__, __language__(32002), 750, __icon__)).encode('utf-8',
                                                                                                                'ignore'))
                xbmc.log(__addonname__ + "=> bad API usage", xbmc.LOGNOTICE)
                ...
                xbmc.executebuiltin(
                    (u'Notification(%s,%s,%s,%s)' % (__addonname__, __language__(32004), 750, __icon__)).encode('utf-8',
                                                                                                                'ignore'))
                xbmc.log(__addonname__ + "=> login or password incorrect", xbmc.LOGNOTICE)
                service[6] = True
            else:
                # everything else
                service = self._service_fail(service, True)
                xbmc.executebuiltin(
                    (u'Notification(%s,%s,%s,%s)' % (__addonname__, __language__(32001), 750, __icon__)).encode('utf-8',
                                                                                                                'ignore'))
                xbmc.log(__addonname__ + "=> server error while authenticating", xbmc.LOGNOTICE)
        return service
    ...
    def _service_mark_as_downloaded(self, service, episode):
        xbmc.log(__addonname__ + "=> ******IN _service_mark_as_downloaded *******", xbmc.LOGNOTICE)
        xbmc.log(__addonname__ + "=> service: " + str(service).strip('[]'), xbmc.LOGDEBUG)
        xbmc.log(__addonname__ + "=> episode: " + str(episode).strip('[]'), xbmc.LOGDEBUG)
        # episode not already marked as downloaded
        if not episode[5]:
            xbmc.log(__addonname__ + "=> Mark show as followed: %s, Show already added: %s" %
            ...
            if service[10] and not episode[6]:
                ...
                try:
                    ...
                    xbmc.log(__addonname__ + "=> failed to follow show %s" % episode[3], xbmc.LOGNOTICE)
                    return service
                # parse results
                if data['errors']:
                    xbmc.log(__addonname__ + "=> %s error : %s %s" % (
                    ...
                    if data['errors'][0]['code'] == 2001:
                        ...
                        service[6] = ''
                        xbmc.log(__addonname__ + "=> bad token while following show", xbmc.LOGNOTICE)
                        return service
                    elif data['errors'][0]['code'] == 2003:
                        xbmc.log(__addonname__ + "=> already following show %s" % episode[3])
                    else:
                        xbmc.executebuiltin((u'Notification(%s,%s,%s,%s)' % (
                            __addonname__, __language__(32005) + episode[3].decode('utf-8'), 750, __icon__)).encode(
                            'utf-8',
                            'ignore'))
                        xbmc.log(__addonname__ + "=> failed to follow show %s" % episode[3], xbmc.LOGNOTICE)
                        return service
                    ...
                    if service[11]:
                        xbmc.executebuiltin((u'Notification(%s,%s,%s,%s)' % (
                            __addonname__, __language__(30013) + episode[3].decode('utf-8'), 750, __icon__)).encode(
                            'utf-8',
                            'ignore'))
                    xbmc.log(__addonname__ + "=> now following show %s" % (episode[3]))
            ...
            try:
                ...
                service = self._service_fail(service, False)
                xbmc.log(__addonname__ + "=> failed to mark as downloaded", xbmc.LOGNOTICE)
                return service
            ...
            if data['errors']:
                ...
                if data['errors'][0]['code'] == 2001:
                    # drop our session key
                    service[6] = ''
                    xbmc.log(__addonname__ + "=> bad token while marking episode", xbmc.LOGNOTICE)
                    ...
                    xbmc.executebuiltin(
                        (u'Notification(%s,%s,%s,%s)' % (__addonname__, __language__(32006), 750, __icon__)).encode(
                            'utf-8',
                            'ignore'))
                    xbmc.log(__addonname__ + "=> error marking episode %s as downloaded" % episode[5], xbmc.LOGNOTICE)
            else:
                if service[11]:
                    xbmc.executebuiltin(
                        (u'Notification(%s,%s,%s,%s)' % (
                            __addonname__, __language__(30014), 750, __icon__)).encode(
                            'utf-8', 'ignore'))
                xbmc.log(__addonname__ + "=> %s episode %s marked as downloaded" % (episode[4], episode[5]))
            return service
...
class MyVideoLibrary(xbmc.Monitor):
    def __init__(self, *args, **kwargs):
        xbmc.log(__addonname__ + "=> MyVideoLibrary.__init__")
        xbmc.Monitor.__init__(self)
        self.action = kwargs['action']
        self.service = kwargs['service']

    def onNotification(self, sender, method, data):
        xbmc.log(__addonname__ + "=> MyVideoLibrary.onNotification")
        if sender == 'xbmc':
            if method == 'VideoLibrary.OnUpdate':
                # if method == 'VideoLibrary.OnScanFinished':
                result = json.loads(data)
                xbmc.log(__addonname__ + "=> " + data)
    ...
    def _get_info(self, episodeid, playcount):
        xbmc.log(__addonname__ + "=> IN ***** MyVideoLibrary._get_info *****")
        try:
            xbmc.log(__addonname__ + "=> Try to get infos from library", xbmc.LOGDEBUG)
            ...
            xbmc.log(__addonname__ + "=> Infos got from library, epname: " + epname, xbmc.LOGDEBUG)
        except:
            xbmc.log(__addonname__ + "=> could not get tvshow/episode details", xbmc.LOGNOTICE)
        if not tvdbid:
            xbmc.log(__addonname__ + "=> No tvdbid", xbmc.LOGDEBUG)

            url = self.service[1] + '/shows/search'
            urldata = '?v=2.4&key=' + self.service[2] + '&title=' + showtitle
            try:
                xbmc.log(__addonname__ + "=> Try to get infos from betaseries", xbmc.LOGDEBUG)

                tvdbid_query = get_urldata(url + urldata, '', "GET")
                tvdbid_query = json.loads(tvdbid_query)
                tvdbid = tvdbid_query['shows'][0]['thetvdb_id']

                xbmc.log(__addonname__ + "=> Infos got from betaseries " + tvdbid, xbmc.LOGDEBUG)
            except:
                xbmc.log(__addonname__ + "=> could not fetch tvshow's thetvdb_id", xbmc.LOGNOTICE)
                return False

        url = self.service[1] + '/shows/search'
        urldata = '?v=2.4&key=' + self.service[2] + '&title=' + showtitle
        try:
            xbmc.log(__addonname__ + "=> Try to get show informations from betaseries " + tvdbid, xbmc.LOGDEBUG)
            tvdbid_query = get_urldata(url + urldata, '', "GET")
            xbmc.log(__addonname__ + "=> " + tvdbid_query, xbmc.LOGDEBUG)
            tvdbid_query = json.loads(tvdbid_query)
            tvdbid = tvdbid_query['shows'][0]['thetvdb_id']
        ...
        try:
            ...
            xbmc.log(__addonname__ + "=> could not fetch episode's thetvdb_id | " + url + urldata, xbmc.LOGNOTICE)
            return False

        epinfo = [int(tvdbid), int(tvdbepid), int(playcount), showtitle, epname, downloaded, showAdded]

        xbmc.log(__addonname__ + "=> OUT ***** MyVideoLibrary._get_info *****")
        return epinfo
...
class MyMonitor(xbmc.Monitor):
    ...
    def onSettingsChanged(self):
        xbmc.log(__addonname__ + "=> onSettingsChanged")
        self.action()


xbmc.log('script started')
